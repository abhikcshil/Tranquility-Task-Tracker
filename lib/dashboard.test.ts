...
test('computeDailyProgress counts due and recurring-today tasks', () => {
  ...
  const tasks = [
    ...
    makeTask({
      id: 2,
      recurringRule: { type: 'daily', frequency: 1, daysOfWeek: null },
      isCompleted: false,
    }),
    makeTask({
      id: 3,
      recurringRule: { type: 'weekdays', frequency: 1, daysOfWeek: '1,3,5' },
      isCompleted: true,
    }),
  ];
...
test('computeWeeklyTaskProgress uses weekly frequency targets', () => {
  const tasks = [
    makeTask({
      ...
      weeklyProgress: { completed: 1, target: 3 },
    }),
    makeTask({
      id: 2,
      recurringRule: { type: 'weekly', frequency: 2, daysOfWeek: null },
      weeklyProgress: { completed: 2, target: 2 },
    }),
  ];
