...
test('selectUpcomingReminders includes incomplete tasks even when dueDate is today', () => {
  const now = new Date('2026-04-13T09:00:00.000Z');
  const reminders = selectUpcomingReminders(
    [
      makeTask({
        ...
        reminderAt: new Date('2026-04-13T22:30:00.000Z'),
      }),
    ],
    now,
  );

  assert.deepEqual(reminders.map((item) => item.id), [9]);
});
